...
new_table(int key_size)
{
    ...
    int init_bulk = MAX(1 << (key_size + 1), 0x1000);
...
gd_open_gif(const char *fname)
{
    ...
    gif->lzw_table = new_table(0x1000);
    goto ok;
fail:
    fd.close();
ok:
...
read_image_data(gd_GIF *gif, int interlace)
{
    ...
    while (1) {
        if (key == clear) {
            ...
            gif->lzw_table->nentries = (1 << (key_size - 1)) + 2;
            table_is_full = 0;
        } else if (!table_is_full) {
            ret = add_entry(&gif->lzw_table, str_len + 1, key, entry.suffix);
            if (ret == -1) {
                // free(table);
                return -1;
            }
            if (gif->lzw_table->nentries == 0x1000) {
                ret = 0;
                table_is_full = 1;
            }
        }
        ...
        if (key == stop) break;
        if (ret == 1) key_size++;
        entry = gif->lzw_table->entries[key];
        ...
        while (1) {
            ...
            if (entry.prefix == 0xFFF)
                ...
                entry = gif->lzw_table->entries[entry.prefix];
        }
        frm_off += str_len;
        if (key < gif->lzw_table->nentries - 1 && !table_is_full)
            gif->lzw_table->entries[gif->lzw_table->nentries - 1].suffix = entry.suffix;
    }
    // free(table);
